...
class DatcomEnv(gym.Env):
    ...
    def __init__(self, transfer = False):
        ...
        self.max_action = np.array([(1.75-1.25)/self.action_size , (3.2-3.0)/self.action_size ,
        ...
        self.action_space = spaces.Box(low = -self.max_action,high = self.max_action, shape=(8,),
            dtype=np.float32
        )

        self.observation_space = spaces.Box(low = self.state_lower, high = self.state_upper,
            dtype=np.float32
        )
    ...
    def step(self, action,normed_state, bıdık):
    
        [self.XLE1, self.XLE2, \
        self.CHORD1_1, self.CHORD1_2, \
        self.CHORD2_1, self.CHORD2_2, \
        self.SSPAN1_2, self.SSPAN2_2]  = np.round(((normed_state + action)*(self.state_upper-self.state_lower) +  self.state_lower), ROUND_FACTOR)
        ...
        #self.done = False 
        self.done =  any(self.state > (np.multiply(self.state_upper,1.001))) or any(self.state < (np.multiply(self.state_lower,0.999)))    
        ...
        if bool(mdict):
            ...
            #self.costs = 100*math.exp(self.cl_cd) - 100*math.exp((self.base_cl_cd))
            #self.costs = 100*(self.cl_cd - self.base_cl_cd)

            #option 1
            #if  self.xcp <= 0.6 and self.xcp >= 0.53 and self.cd<self.base_cd:
            #    self.costs = 100*(self.cl_cd - self.base_cl_cd)
            #else:
            #    self.costs = 100*(self.cl_cd - self.base_cl_cd) - 1000 * abs(self.xcp - self.base_xcp) - 1000 * abs(self.cd - self.base_cd)
            #option 2
            #if  self.xcp > 0.6 and self.xcp < 0.53:
            #    self.costs = 100*(self.cl_cd - self.base_cl_cd) - 1000 * abs(self.xcp - self.base_xcp)
            #elif  self.cd>=self.base_cd:                
            #    self.costs = 100*(self.cl_cd - self.base_cl_cd) - 1000 * abs(self.cd - self.base_cd)
            #else:
            #    self.costs = 100*(self.cl_cd - self.base_cl_cd)
            # option 3
            if (self.xcp > 0.6 or self.xcp < 0.53) and self.cd<self.base_cd:
            
            	self.costs = 100*(self.cl_cd - self.base_cl_cd) - 100*bıdık * abs(self.xcp - self.base_xcp)

            elif self.cd>=self.base_cd and (self.xcp <= 0.6 and self.xcp >= 0.53):

            	self.costs = 100*(self.cl_cd - self.base_cl_cd) - 100*bıdık * abs(self.cd - self.base_cd)

            elif self.cd>=self.base_cd and (self.xcp > 0.6 or self.xcp < 0.53):

            	self.costs = 100*(self.cl_cd - self.base_cl_cd) - 100*bıdık * abs(self.cd - self.base_cd) - 100*bıdık * abs(self.xcp - self.base_xcp)

            else:

            	self.costs = 100*self.cl_cd - 100*self.base_cl_cd
        ...
        return self.state, self.costs, self.done, {}
